	...
	pub fn prepare_read(
		...
		match self.timeseries.get_bounds(t_start, t_end) {
			...
			BoundResult::Ok((start_byte, stop_byte, decode_params)) => {
				if stop_byte - start_byte == 0 {
					return None;
				}

				let fields: Vec<MetaField<f32>> = requested_fields
					.into_iter()
					.map(|id| self.metadata.fields[*id as usize].clone())
					.collect();

				let numb_lines =
					(stop_byte - start_byte) / (self.timeseries.full_line_size as u64) - 1;
				let decoded_line_size =
					fields.len() * std::mem::size_of::<f32>() + std::mem::size_of::<f64>();
...
pub fn prepare_read_processing(
	...
	if max_points == 0 {
		warn!("cant create a plot with zero points");
		return None;
	}
	...
	let selector =
		minimal_timeseries::Selector::new(max_points as usize, lines_in_range as u64, &timeseries);
	...
	*/
	...
	let lines_per_package = usize::max(
		lines_per_sample,
		IDEAL_READ_SIZE / bytes_to_read_per_packet_line,
	);
...
pub fn read_into_packages(
	data_handle: Arc<RwLock<Data>>,
	mut tx: mpsc::SyncSender<Vec<u8>>,
	mut reader_infos: Vec<ReaderInfo>,
) {
	...
	for mut reader in reader_infos.iter_mut().filter(|x| x.selector.is_none()) {
		...
		dataset
			...
			.expect("timeseries read returned no data");
	...
	while sets_to_process > 0 {
		for mut reader in reader_infos.iter_mut().filter(|x| !x.done) {
			...
			dataset
				.timeseries
				.decode_time_into_given_skipping(
					&mut reader.timestamps_buffer,
					&mut reader.line_data_buffer,
					reader.lines_per_read, //max number of lines to read, is a multiple of lines_per_sample
					&mut reader.read_state.start_byte,
...
impl ReaderInfo {
	...
	fn empty_into_packages(
		&mut self,
		tx: &mut mpsc::SyncSender<Vec<u8>>,
		...
		let mut bytes_to_fill_package =
			(self.bytes_per_package - self.package.len()) * lines_per_sample;
		...
		while ldate_remainder.len() >= bytes_to_fill_package {
			...
			let (tstamp_left, new_tstamp_remainder) =
				tstamp_remainder.split_at(lines_to_fill_package);
			let (ldata_left, new_ldate_remainder) = ldate_remainder.split_at(bytes_to_fill_package);
			...
			let next_package =
				Self::new_package(self.package_numb, self.dataset_id, self.bytes_per_package); //prepare next package
																			   //replace self.package with next package and send the old self.package
			if tx
				.send(mem::replace(&mut self.package, next_package))
				.is_err()
			{
	...
	fn decode_into_package(
		...
		for sample in ldata.chunks_exact(lines_per_sample * line_size) {
			...
			for decoded_sum in decoded_field_sums.drain(..) {
				package
					.write_f32::<LittleEndian>(decoded_sum / lines_per_sample as f32)
					.unwrap(); //FIXME actually do an average here
			}
		} //for every sample
	}
}
