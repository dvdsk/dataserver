...
/*pub fn handle_bot_message<T: InnerState+'static>(state: Data<T>, update: Form<Update>)
	...
	dbg!(&update);
	match &update.kind{
		UpdateKind::Message(message) => send_test_reply(&message.chat).unwrap(),
...
fn send_test_reply<C: ToChatRef>(chat: C) -> Result<(), BotError>{//add as arg generic ToChatRef (should get from Update)
